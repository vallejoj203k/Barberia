...
def seed():
    """Seed the database. Must be called within an active app context."""
    from models import db, User, Barber, Service, Appointment, LoyaltyCard, Notification
    ...
    for client, barber_obj, service, day_offset, slot, status in week_data:
        appt_date = monday + timedelta(days=day_offset)
        if appt_date.weekday() == 6:
            continue
        appt = Appointment(
            client_id=client.id,
            barber_id=barber_obj.id,
    ...
    # ---- LOYALTY CARDS ----
    db.session.add_all([
        LoyaltyCard(client_id=lucas.id, total_cuts=4, free_cuts_redeemed=1),
        LoyaltyCard(client_id=sofia.id, total_cuts=8, free_cuts_redeemed=1),
        LoyaltyCard(client_id=martin.id, total_cuts=3, free_cuts_redeemed=0),
        LoyaltyCard(client_id=camila.id, total_cuts=6, free_cuts_redeemed=1),
        LoyaltyCard(client_id=nicolas.id, total_cuts=2, free_cuts_redeemed=0),
        LoyaltyCard(client_id=valentina.id, total_cuts=5, free_cuts_redeemed=0),
    ])
